...
class test_enforce_tax_account(common.TransactionCase):
    def setUp(self):
        ...
        self.period = self.env['account.period'].with_context(
            company_id=self.journal.company_id.id).find()
        ...
        self.tax_code = self.env['account.tax.code'].search(
            [('company_id', '=', self.journal.company_id.id)])[0]
        self.other_tax_code = self.env['account.tax.code'].search(
            [('id', '!=', self.tax_code.id),
             ('company_id', '=', self.journal.company_id.id)])[0]
    ...
    def test_create_fail_when_no_tax(self):
        ...
        expected_args = _('ValidateError'),\
            _('Error while validating constraint\n\nAccount %s '
                'does not permit postings without VAT code.') % \
            self.account.code
    ...
    def test_create_fail_when_wrong_tax(self):
        ...
        expected_args = _('ValidateError'), \
            _('Error while validating constraint\n\n'
                ...
                'tax code "%s"' % (self.account.code,
                                   self.other_tax_code.name))
        self.assertEqual(the_exception.args, expected_args)
